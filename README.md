...
| Every column ▣ | Added / Missing / Changed ▣ | Before-after values ▣ | CSV export ▣ |
|---|---|---|---|
...
identity - the credentials are encrypted before they ever touch disk.
...
2. Choose the snapshot - the exported spreadsheet. Its columns, its values,
   its truth.
3. Compare. Read the verdict: every column, every value matches - or X
   items drifted.
...
`drugitems-core`. The design language lives in `docs/DESIGN.md`.
